...
stderr <- function(vec) #std dev/sample size
{
...
temp<-glmer(cbind(Hatch.primary,Test.eggs.primary)~Stimulus+ (1|Clutch), family=binomial(logit), data=younger2018, control=glmerControl(optimizer="bobyqa"))
Anova(temp)

#binomial glm for OLDER
temp<-glmer(cbind(Hatch.primary,Test.eggs.primary)~Stimulus+ (1|Clutch), family=binomial(logit), data=older2018, control=glmerControl(optimizer="bobyqa"))
Anova(temp)
```
