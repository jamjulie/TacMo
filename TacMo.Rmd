...
```{r}
data2017 <- read.csv(file="TacMo_2017_Jung20180826_clean.csv")
head(data2017)
```
...
```{r}
str(data2017)
min(data2017$First_Hatch, na.rm=T)
max(data2017$First_Hatch, na.rm=T)
```
...
```{r}
hist(data2017$First_Hatch) #nonparametric
hist(log(data2017$First_Hatch)) #parametric!
```
...
```{r}
# IN SECONDS, Anova
aov1 <- aov(log(First_Hatch) ~ Age.day*Stimulus, data=data2017)
summary(aov1)
```
...
LtoH_younger_errorstats2017<-
  ...
  group_by(Stimulus) %>%
  summarize(count = n(),
            mean = mean(First_Hatch, na.rm=T),
            SD = sd(First_Hatch, na.rm=T), 
            SE = sd(First_Hatch, na.rm=T)/sqrt(n())
            )
...
```{r}
str(data2018)
min(data2018$First_Hatch , na.rm=T)
max(data2018$First_Hatch , na.rm=T)
```
...
```{r}
hist(data2018$First_Hatch) #nonparametric
hist(log(data2018$First_Hatch)) #parametric!
```
Since they are, we can use ANOVAs of log-transformed data to test for 
effects of age, stimulus and age-by-stimulus interaction 
on the latency to hatch. 
...
```{r}
# IN SECONDS, Anova
aov1 <- aov(log(First_Hatch) ~ Age.day*Stimulus, data=data2018)
summary(aov1)
```
> Latency varied among stimuli and decreased with age, with a marginally non-significant interaction effect (Fig. 6B,C). This pattern held whether measuring latency in seconds or in cycles of vibration (seconds: age, F1,58 = 39.52, P < 0.0001; stimulus, F2,58 = 13.72, P < 0.0001; interaction, F2,58 = 2.91, P = 0.0625; cycles: age, F1,58 = 49.54, P < 0.0001; stimulus, F2,58 = 71.89, P < 0.0001; interaction, F2,58 = 2.91, P = 0.0625).
